...
begin
  if data.validate then
    with pool.GetConnection.Transaction do
     Execute(Query('update blog set title = ?, body = ? where id = ?'),
...
begin
  with pool.GetConnection.Transaction do
    Execute(Query('delete from blog where id = ?'), id);
  Redirect('blog', 'index');
end;
...
begin
  with pool.GetConnection.Transaction do
    data := Execute(Singleton('select * from blog where id = ?'), id);
end;

procedure TBlogController.Index_get(out data: ISuperObject);
begin
  with pool.GetConnection.Transaction do
    data := Execute(Query('select title, id, post_date from blog order by post_date'));
end;
...
begin
  if data.validate then
    with pool.GetConnection.Transaction do
     Redirect(Execute(Singleton('insert into blog (title, body) values (?, ?) returning id'),
