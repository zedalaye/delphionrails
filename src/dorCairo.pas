...
interface
uses sysutils, classes, dorCairolib
{$IFDEF WIN32}
, windows
{$ENDIF}
...
{$endif}
  ...
  ['{EA646BC8-2E6A-4288-BBA2-C68AE29795EB}']
    function GetStatus: TCairoStatus;
    function GetType: TCairoFontType;
    procedure Extents(const extent: TCairoFontExtents);
    procedure TextExtents(const utf8: UTF8String; const extent: TCairoTextExtents);
    procedure GlyphExtents(glyphs: PCairoGlyph; numGlyphs: Integer; const extent: TCairoTextExtents);
    procedure TextToGlyphs(x, y: Double; const utf8: UTF8String = '';
  ...
  protected
    function UserTextToGlyphs(const utf8: PAnsiChar; utf8Len: Integer; glyphs: PPCairoGlyph; numGlyphs: PInteger; clusters: PPCairoTextCluster; numClusters: PInteger; clusterFlags: PCairoTextClusterFlags): TCairoStatus; virtual; abstract;
...
{$ifdef CAIRO_HAS_RSVG_FUNCTIONS}
    ...
    constructor CreateFromSVG(const stream: TStream; format: TCairoFormat; scale: double); overload;
{$endif}
  end;
...
{$ifdef CAIRO_HAS_SVG_SURFACE}
  ...
  public
    ...
    constructor Create(const filename: TFileName; widthInPoints, heightInPoints: Double); overload;
    destructor Destroy; override;
  end;
{$endif}
...
{$endif}
  ...
  ICairoContext = interface
    ...
    procedure ShowTextGlyphs(const utf8: UTF8String; const glyphs: PCairoGlyph;
      numGlyphs: Integer; const clusters: PCairoTextCluster; numClusters: Integer;
      clusterFlags: TCairoTextClusterFlags);
    procedure TextPath(const utf8: UTF8String);
    procedure GlyphPath(const glyphs: PCairoGlyph; numGlyphs: Integer);
    procedure TextExtents(const utf8: UTF8String; extent: PCairoTextExtents);
    procedure GlyphExtents(const glyphs: PCairoGlyph; numGlyphs: Integer; extent: PCairoTextExtents);
...
constructor TImageSurface.CreateFromSVG(const stream: TStream; format: TCairoFormat; scale: double);
var
  handle: PRsvgHandle;
...
procedure TCairoContext.GlyphExtents(const glyphs: PCairoGlyph;
  numGlyphs: Integer; extent: PCairoTextExtents);
begin
  cairo_glyph_extents(FContext, glyphs, numGlyphs, extent);
end;

procedure TCairoContext.GlyphPath(const glyphs: PCairoGlyph;
  numGlyphs: Integer);
begin
...
procedure TCairoContext.ShowText(const utf8: UTF8String);
begin
  cairo_show_text(FContext, PAnsiChar(utf8));
end;

procedure TCairoContext.ShowTextGlyphs(const utf8: UTF8String;
  const glyphs: PCairoGlyph; numGlyphs: Integer; const clusters: PCairoTextCluster;
...
procedure TCairoContext.TextExtents(const utf8: UTF8String;
  extent: PCairoTextExtents);
begin
  cairo_text_extents(FContext, PAnsiChar(utf8), extent);
end;

procedure TCairoContext.TextPath(const utf8: UTF8String);
begin
...
procedure TCairoScaledFont.Extents(const extent: TCairoFontExtents);
begin
...
procedure TCairoScaledFont.GlyphExtents(glyphs: PCairoGlyph; numGlyphs: Integer;
  const extent: TCairoTextExtents);
begin
...
procedure TCairoScaledFont.TextExtents(const utf8: UTF8String;
  const extent: TCairoTextExtents);
begin
  cairo_scaled_font_text_extents(FScaledFont, PAnsiChar(utf8), @extent);
end;

procedure TCairoScaledFont.TextToGlyphs(x, y: Double; const utf8: UTF8String;
...
function cairoUserScaledFontTextToGlyphs(scaledFont: PCairoScaledFont; const utf8: PAnsiChar; utf8Len: Integer; glyphs: PPCairoGlyph; numGlyphs: PInteger; clusters: PPCairoTextCluster; numClusters: PInteger; clusterFlags: PCairoTextClusterFlags): TCairoStatus; cdecl;
begin
