...
function HTTP_ParseURL(const uri: PChar; out protocol, domain: AnsiString; out port: Word; out get: string): Boolean;
type
  TState = (sStart, sDomain, sPort);
var
  p, d, dot1, dot2: PChar;
  ...
  begin
    ...
    begin
      SetLength(domain, len);
      PunycodeEncode(dot1-dot2, PPunyCode(dot2), len, PByte(domain));
      domain := AnsiString(Copy(d, 0, dot2 - d)) + 'xn--' + domain + AnsiString(Copy(dot1, 0, p - dot1));
    end else
      SetString(domain, d,  p - d);
  end;
begin
      ...
      case s of
        sStart:
          case o of
            ...
            2: if (p^ = ':') then Inc(o) else Exit(False);
            3: if (p^ = '/') then Inc(o) else Exit(False);
            4: if (p^ = '/') then begin s := sDomain; o := 0; pushdot end else Exit(False);
          end;
        sDomain:
          case o of
            0:
